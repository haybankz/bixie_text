  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (_, index) => ListTile(
        ...
        trailing: Radio<String>(value: widget.options[index].name!, onChanged: (value){
          setState(() {
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (_, index) => ListTile(
        ...
        title: Text(widget.options[index].name!),
        tileColor: selectedOptions.contains(widget.options[index].name)? Colors.red : Colors.white,
        onTap: () {
          setState(() {
            if(selectedOptions.contains(widget.options[index].name)){
              ...
              if(selectedOptions.length < widget.maximumOption) {
                selectedOptions = [...selectedOptions, widget.options[index].name!];
              }else{
                Fluttertoast.showToast(msg: "Maximum number of allowed options is '${widget.maximumOption}'");
              }
            }
            widget.onSelect(widget.options.where((element) => selectedOptions.contains(element.name)).toList());
          });
        },
      ),
    );
